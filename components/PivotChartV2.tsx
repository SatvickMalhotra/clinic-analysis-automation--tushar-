...
const PivotChartV2 = React.forwardRef<HTMLDivElement, PivotChartProps>(({ data, title, isHorizontal = false }, ref) => {
    ...
    const horizontalLayout = (
        <BarChart layout="vertical" data={data} margin={{ top: 5, right: 30, left: 100, bottom: 20 }}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis type="number" yAxisId="left" orientation="bottom" stroke="#4f46e5" tickFormatter={formatAxisTick} tick={{ fontSize: 12 }} />
            <XAxis type="number" yAxisId="right" orientation="top" stroke="#10b981" tickFormatter={(val) => val.toLocaleString('en-IN')} tick={{ fontSize: 12 }}>
